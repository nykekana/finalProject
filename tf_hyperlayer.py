...
class LookupLayer(layers.Layer):
    def __init__(self, in_ch, out_ch, num_objects):
        ...
        self.out_ch = out_ch
        self.lookup_lin = layers.LookupLinear(in_ch,
                                       out_ch,
                                       num_objects=num_objects)
        self.norm_nl = Sequential(
            layers.LayerNormalization([self.out_ch]),
            layers.ReLU(inplace=True)
        )

    def call(self, obj_idx):
        net = Sequential(
...
class FCLayer(layers.Layer):
    def __init__(self, in_features, out_features):
        ...
        self.net = Sequential(
            ...
            LayerNormalization([out_features]),
            ReLU(inplace=True)
        )
...
class LookupLinear(layers.Layer):
    def __init__(self,
                 in_ch,
                 out_ch,
                 num_objects):
        ...
        self.hypo_params = layers.Embedding(num_objects, in_ch * out_ch + out_ch)
    ...
    def forward(self, obj_idx):
        ...
        # Indices explicit to catch errors in shape of output layer
...
class HyperLayer(layers.Layer):
    ...
    def forward(self, hyper_input):
        ...
        '''
        return Sequential(self.hyper_linear(hyper_input), self.norm_nl)

class HyperFC(layers.Layer):
...
class BatchLinear(layers.Layer):
    def __init__(self,
                 weights,
                 biases):
    ...
    def forward(self, input):
        output = tf.linalg.matmul(self.weights.permute(*[i for i in range(len(self.weights.shape)-2)], -1, -2))
        output += self.biases
        return output

class HyperLinear(layers.Layer):
